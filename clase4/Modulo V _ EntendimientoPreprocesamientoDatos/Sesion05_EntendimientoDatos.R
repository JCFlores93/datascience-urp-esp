...
# Tema: Entendimiento - Preprocesamiento Datos
# Naive Bayes - KNN
# version: 1.0
#########################################################################
...
library(caret)  ## Modelosde machine learning, Tunning de parametros
library(DataExplorer) ## Missings y exploracion de los datos
library(VIM) ## Missings, preprocesamiento y exploracion de los datos
library(missForest)  ## Imputacion por Random Forest
library(ggplot2)  ##  Graficos
library(dummies)  ## Binarizacion
library(iplots)  ## Visualizacion y preprocesamiento
...
# Eliminando la columna de identificaci?n del cliente (Loan_ID)
datos$Loan_ID<-NULL
...
# knnImpute -> Garantizo que el dataset est� lleno 
# "center","scale" -> Transformo linealmente variables cuantitativas
...
library(missForest)
set.seed(123)
# Solamente trabajar� con los valores cuantitativos
...
PlotMissing(datos_transformado1)
...
# Debemos validar con zeroVar y nzv
# Cuando la varianza es muy alta. 
...
PlotMissing(datos_transformado2)
...
PlotMissing(censusn_transformado2)
